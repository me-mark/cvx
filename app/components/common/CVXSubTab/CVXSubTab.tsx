...
export function CVXSubTab(props: CVXTabModel) {
  ...
  useEffect(() => {
    const widthOfContent = document.getElementsByClassName("status-wrapper")[0]
      .clientWidth;
    const widthOfParent = document.getElementsByClassName("cvx-sub-tab__wrapper__content")[0]
      .clientWidth;
  ...
  return (
    <div className="cvx-sub-tab__wrapper">
      <div className="cvx-sub-tab__wrapper__content">
        <div className="status-wrapper flex-layout margin-4" style={style}>
          {tabData.map((item: any, index: number) => (
            <div
              className={
                activeTab === item.id
                  ? "status cvx-sub-tab__wrapper__content__tab select-active center active"
                  : "status cvx-sub-tab__wrapper__content__tab select-active center"
              }
              onClick={() => handleClickTab(item)}
              key={index}
            >
              <h2 className="title-1 cvx__main-content relative cvx-subtab__title">
                {item.label}
              </h2>
            </div>
          ))}
      ...
      {scrollable && positionX < 0 && (
        <div className="cvx-sub-tab__wrapper__last-button" onClick={handleClickPrevious}>
          <FontAwesomeIcon icon={faAngleLeft} color="currentColor" size="3x" />
        </div>
      )}
      {scrollable && positionX >= 0 && (
        <div className="cvx-sub-tab__wrapper__next-button" onClick={handleClickNext}>
